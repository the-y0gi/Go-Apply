...
// const toast = {
//   success: (msg: string) => { try { window.alert(msg) } catch { console.log("SUCCESS:", msg) } },
//   error: (msg: string) => { try { window.alert(msg) } catch { console.error("ERROR:", msg) } }
// }
...
//         const res = await axios.get<{ success: boolean; data: { programs: Program[] } }>(
...
//             state: uni.country ?? "",
//             duration: p.duration ?? "",
//             tuition: tuitionAmount ? `${currency} ${tuitionAmount.toLocaleString()}` : "N/A",
//             ranking: uni.ranking?.global ? `#${uni.ranking.global}` : (p.ranking ?? ""),
...
//      () => {
//        try {
//         return JSON.parse(localStorage.getItem("appliedPrograms") || "[]") } catch { return [] }
//      }
//    )
...
//    const { programs: allPrograms, loadingPrograms, programsError } = usePrograms()
...
//      const tuitionValue = Number(String(program.tuition).replace(/[^\d.]/g, "")) || 0
//      const matchesTuition =
//        tuitionValue >= (tuitionRange[0] || 0) && tuitionValue <= (tuitionRange[1] || Infinity)

//      // Normalize rating to number
//      const ratingValue =
//        typeof program.rating === "number" ? program.rating : parseFloat(String(program.rating)) || 0
...
//          (program.tags || []).some((tag) => tag.toLowerCase().includes(feature.toLowerCase()))
//        )
...
//      if (appliedPrograms.includes(program.id) || applyingIds.includes(program.id)) return
...
//            try { localStorage.setItem("appliedPrograms", JSON.stringify(next)) } catch {}
//            return next
//          })
...
//        const docs = Array.isArray(requirements.documentsRequired) ? requirements.documentsRequired : []
//        const prereq = Array.isArray(requirements.prerequisites) ? requirements.prerequisites : []
//        const tests = Array.isArray(requirements.englishTests)
//          ? requirements.englishTests.map((t: any) => `${t.testType}: ${t.minScore}`)
//          : []
//        const extras: string[] = []
//        if (requirements.minGPA) extras.push(`GPA: ${requirements.minGPA}`)
//        if (requirements.workExperience?.required) {
//          extras.push(`Work Exp: ${requirements.workExperience.minYears || "N/A"} yrs`)
//        }
//        return [...docs, ...prereq, ...tests, ...extras].filter(Boolean)
//      }
//      return String(requirements).split(",").map((s) => s.trim()).filter(Boolean)
//    }
...
//                                      <span key={String(req)} className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded">
//                                        {req}
//                                      </span>
//                                    ))}
//                                  </div>
